...
# # docker build -t mafl_api .
# # docker run -d -p 80:80 -e RAILS_MASTER_KEY=<value from config/master.key> --name mafl_api mafl_api
...
# ----------------------------------
    ...
    # Copy the app and gems from build stage
    COPY --from=build "${BUNDLE_PATH}" "${BUNDLE_PATH}"
    COPY --from=build /rails /rails
